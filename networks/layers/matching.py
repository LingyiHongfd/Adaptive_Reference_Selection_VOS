...
def _nn_features_per_object_for_chunk(
        reference_embeddings, ref_square, query_embeddings, query_square, wrong_label_mask, n_trucks, hw):
    ...
    features, argmin_idx = torch.min(dists, 2, keepdim=True)

    if n_trucks == 1:
        H, W = hw
        y_idx, x_idx = argmin_idx // W, argmin_idx % W
        g = make_gaussian(y_idx, x_idx, H, W, sigma=5.6).permute(1, 0, 2)
        dists = dists * (g)
...
def _nearest_neighbor_features_per_object_in_chunks(
        reference_embeddings_flat, query_embeddings_flat, reference_labels_flat, n_chunks, hw):
...
def global_matching(
    ...
    if ori_size is not None:
        ...
        nn_features_reshape = F.interpolate(nn_features_reshape, size=ori_size,
                                            mode='bilinear', align_corners=True).permute(2, 3, 0, 1).view(1,
                                                                                                          ori_size[0],
                                                                                                          ori_size[1],
                                                                                                          obj_nums, 1)
...
def global_matching_for_eval(
    ...
    if ref_num == 1:
        ...
        for reference_embeddings, reference_labels, idx in zip(all_reference_embeddings, all_reference_labels,
                                                               range(ref_num)):
            if atrous_rate > 1:
                h_pad = (atrous_rate - h % atrous_rate) % atrous_rate
                w_pad = (atrous_rate - w % atrous_rate) % atrous_rate
    ...
    nn_features = _nearest_neighbor_features_per_object_in_chunks(
        reference_embeddings_flat, query_embeddings_flat, reference_labels_flat, n_chunks,(h, w))
...
def local_matching(
    ...
    offset_masks = F.unfold(padded_labels, kernel_size=(height, width),
                            stride=(atrous_rate, atrous_rate)).view(obj_num, height, width, -1).permute(1, 2, 3,
                                                                                                        0) > 0.9
