...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.means = torch.Tensor([0.229, 0.224, 0.225]).cuda()
        self.std = torch.Tensor([0.485, 0.456, 0.406]).cuda()
    ...
    def image_deform(self, sample):
        sample = sample.permute(0, 2, 3, 1)
        sample = sample * self.means
        sample = sample + self.std
        sample = sample * 255.
        sample = sample.permute(0, 3, 1, 2).int().float()
        return sample
    ...
    def forward(self, x, size):
        ...
        viz=Visdom()
        x = self.image_deform(x)
        viz.image(x.squeeze(0))
        x = x.cuda()
        x = self.conv1(x)  # 1/2
        x = self.bn1(x)
        x = self.relu(x)
...
def contour_model(pretrained=False, **kwargs):
    ...
    model.load_state_dict(
        torch.load('./pretrain_model/lr-0.01-iter-490000.pth'))
    return model
...
class Contour_Conv(nn.Module):
    def __init__(self):
        super(Contour_Conv, self).__init__()
        self.contour_model = contour_model().cuda()
        self.contour_conv1 = nn.Conv2d(2, 2, kernel_size=3, stride=1, padding=1, )
        self.contour_bn = nn.BatchNorm2d(2)
        self.contour_relu = nn.ReLU(True)
        self.contour_conv2 = nn.Conv2d(2, 1, kernel_size=1, stride=1, )
        self.contour_conv = nn.Sequential(*[self.contour_conv1, self.contour_bn, self.contour_relu, self.contour_conv2])

    def forward(self, input):
        return self.contour_conv(input)
